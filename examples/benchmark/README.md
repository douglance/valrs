...
| **yup** | Established validation library |
...
This benchmark runs in Node.js using valrs's pure JavaScript validation. The WASM module requires a browser environment (uses `fetch()` to load the `.wasm` binary).

For WASM benchmarks in a browser environment, the performance characteristics may differ.
