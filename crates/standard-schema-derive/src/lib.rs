...
//!
//! Provides `#[derive(StandardSchema)]` and `#[derive(StandardJsonSchema)]` for automatic implementation.
...
/// Derives the `StandardSchema` trait for a struct.
///
/// # Example
///
/// ```ignore
/// use standard_schema::StandardSchema;
/// use standard_schema_derive::StandardSchema;
///
/// #[derive(StandardSchema)]
/// pub struct User {
///     pub name: String,
///     pub age: u32,
/// }
...
#[proc_macro_derive(StandardSchema, attributes(schema))]
pub fn derive_standard_schema(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);

    match derive_standard_schema_impl(input) {
        Ok(tokens) => tokens.into(),
        Err(e) => e.to_compile_error().into(),
    }
}

fn derive_standard_schema_impl(input: DeriveInput) -> syn::Result<TokenStream2> {
        ...
        Data::Struct(data) => match &data.fields {
            ...
            Fields::Unnamed(_) => {
                return Err(Error::new_spanned(
                    struct_name,
                    "StandardSchema derive does not support tuple structs",
                ))
            }
            Fields::Unit => {
                return Err(Error::new_spanned(
                    struct_name,
                    "StandardSchema derive does not support unit structs",
                ))
            }
        },
        Data::Enum(_) => {
            return Err(Error::new_spanned(
                struct_name,
                "StandardSchema derive does not support enums yet",
            ))
        }
        Data::Union(_) => {
            return Err(Error::new_spanned(
                struct_name,
                "StandardSchema derive does not support unions",
            ))
        }
    };
    ...
    let expanded = quote! {
        impl ::standard_schema::StandardSchema for #struct_name {
            type Input = #struct_name;
            type Output = #struct_name;

            fn validate(value: &::serde_json::Value) -> ::standard_schema::ValidationResult<Self::Output> {
                let obj = match value.as_object() {
                    Some(o) => o,
                    None => return ::standard_schema::ValidationResult::failure("Expected object"),
                };

                let mut issues: Vec<::standard_schema::ValidationIssue> = Vec::new();

                #(#field_validations)*

                if !issues.is_empty() {
                    return ::standard_schema::ValidationResult::Failure(issues);
                }

                ::standard_schema::ValidationResult::Success(#struct_name {
                    #(#field_names),*
                })
            }
        }
    ...
    if field.attrs.optional {
        ...
        if let Some(inner_ty) = extract_option_inner_type(field_ty).filter(|_| has_length_validations) {
            ...
            quote! {
                let #field_ident: #field_ty = match obj.get(#json_key) {
                    ...
                    Some(v) => {
                        match <#inner_ty as ::standard_schema::StandardSchema>::validate(v) {
                            ::standard_schema::ValidationResult::Success(inner_val) => {
                                // Apply length validations to the inner value
                                let val = &inner_val;
                                #length_validations
                                Some(inner_val)
                            }
                            ::standard_schema::ValidationResult::Failure(errs) => {
                                for mut err in errs {
                                    let mut new_path = vec![::standard_schema::PathSegment::Key(#json_key.to_string())];
            ...
            quote! {
                let #field_ident: #field_ty = match obj.get(#json_key) {
                    Some(::serde_json::Value::Null) | None => None,
                    Some(v) => {
                        match <#field_ty as ::standard_schema::StandardSchema>::validate(v) {
                            ::standard_schema::ValidationResult::Success(val) => val,
                            ::standard_schema::ValidationResult::Failure(errs) => {
                                for mut err in errs {
                                    let mut new_path = vec![::standard_schema::PathSegment::Key(#json_key.to_string())];
                                    if let Some(existing_path) = err.path.take() {
        ...
        quote! {
            let #field_ident: #field_ty = match obj.get(#json_key) {
                Some(v) => {
                    match <#field_ty as ::standard_schema::StandardSchema>::validate(v) {
                        ::standard_schema::ValidationResult::Success(validated_val) => {
                            #length_block
                            validated_val
                        }
                        ::standard_schema::ValidationResult::Failure(errs) => {
                ...
                None => {
                    issues.push(::standard_schema::ValidationIssue::with_path(
                        format!("Missing required field '{}'", #json_key),
                        vec![::standard_schema::PathSegment::Key(#json_key.to_string())],
                    ));
                    // Use default to allow continuing validation of other fields
                    <#field_ty as Default>::default()
                }
...
///
/// ```ignore
/// use standard_schema::{StandardSchema, StandardJsonSchema, JsonSchemaTarget};
/// use standard_schema_derive::{StandardSchema, StandardJsonSchema};
///
/// #[derive(StandardSchema, StandardJsonSchema)]
/// pub struct User {
///     pub name: String,
///     #[schema(min_length = 1)]
///     pub email: String,
...
fn derive_standard_json_schema_impl(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let expanded = quote! {
        impl ::standard_schema::StandardJsonSchema for #struct_name {
            fn json_schema_input(target: ::standard_schema::JsonSchemaTarget) -> ::serde_json::Value {
            ...
            fn json_schema_output(target: ::standard_schema::JsonSchemaTarget) -> ::serde_json::Value {
        ...
        quote! {
            {
                ...
                let mut prop_schema = <#schema_ty as ::standard_schema::StandardJsonSchema>::json_schema_input(
                    ::standard_schema::JsonSchemaTarget::OpenApi30
                );
